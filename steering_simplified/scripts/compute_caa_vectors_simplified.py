...
Prerequisites:
  - outputs_simplified/activations.npy (11708, 28, 3584)
  - outputs_simplified/labels.npy (11708,)
...
activations = np.load("outputs_simplified/activations.npy")
labels = np.load("outputs_simplified/labels.npy")
labels_str = np.array([LABEL_MAP[i] for i in labels])
print(f"Shape: {activations.shape}")
print(f"Labels: { {k: int((labels_str == k).sum()) for k in LABEL_MAP.values()} }")

out_dir = Path("steering_vectors_simplified")
